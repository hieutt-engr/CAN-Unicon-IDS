...
#     def forward(self, x):
#         # Đầu vào: [batch_size, channels, height, width]
#         batch_size, C, H, W = x.size()
        
#         # Xem từng hình ảnh là một step (time_steps = batch_size)
#         x = x.view(batch_size, 1, C, H, W)  # Thêm trục time_steps
#         time_steps = x.size(0)
        
#         # CNN feature extraction cho từng step
#         cnn_features = []
#         for t in range(time_steps):
#             out = self.pool(F.relu(self.conv1(x[t, :, :, :, :])))
#             out = self.pool(F.relu(self.conv2(out)))
#             out = self.pool(F.relu(self.conv3(out)))
#             out = out.view(out.size(0), -1)  # Flatten thành [1, feature_dim]
#             cnn_features.append(out)
        
#         # Stack CNN features: [time_steps, batch_size, feature_dim]
#         cnn_features = torch.stack(cnn_features, dim=0)
        
#         # LSTM: [time_steps, batch_size, lstm_hidden_dim]
#         lstm_out, _ = self.lstm(cnn_features)
        
#         # Lấy đầu ra từ step cuối: [batch_size, lstm_hidden_dim]
#         lstm_out = lstm_out[-1, :, :]
...
#     def forward(self, x):
#         # Đầu vào: [batch_size * 2, channels, height, width]
#         batch_size = x.size(0)
...
#         out = self.pool(F.relu(self.conv2(out)))
#         out = self.pool(F.relu(self.conv3(out)))
#         out = out.view(out.size(0), -1)  # Flatten: [batch_size * 2, feature_dim]
        
#         # Add time_steps dimension for LSTM
#         out = out.unsqueeze(1)  # [batch_size * 2, 1, feature_dim]
        
#         # Pass through LSTM
#         lstm_out, _ = self.lstm(out)  # [batch_size * 2, 1, lstm_hidden_dim]
#         lstm_out = lstm_out[:, -1, :]  # Take the last step: [batch_size * 2, lstm_hidden_dim]
        
#         # Fully connected layers
#         embedding = self.fc(lstm_out)  # [batch_size * 2, embedding_dim]
